...
public class FightingControl : MonoBehaviour
{
   ...
   // public GameObject enemyObject;  // 怪物角色
    ...
    public string battleDescTxt;   //储存战斗描述内容
    public string battleRoundNumTxt;  //储存战斗回合数文本内容
    ...
    public void BattleProcessCalculation(RoleInfo role,RoleInfo enemy,int rounds=1)
    {
        ...
        Debug.Log(txt);
        battleRoundNumTxt=battleRoundNumTxt+txt+"\n";
        // playerA发动一次攻击
        ...
        if(playerA.roleHp<=0)
        {
            ...
            Debug.Log(txt);
            battleDescTxt=battleDescTxt+txt+"\n";
            UpdateBattleDescText(battleDescTxt,battleRoundNumTxt);
        }
    ...
    public Tuple<int,int,string,bool> ReleaseMagicDamageCalculate(RoleInfo atkRole,RoleInfo dmgRole)
    {
        ...
        if(RandomDetermine(0.33f))
        {
            ...
            consumeMana=100;
            magicName="火球术";
            finaldamage=1.5f*atkRole.roleMagicAttack; //该法术造成1.5f倍率的法术伤害
        ...
        else if(RandomDetermine(0.5f))
        {
            consumeMana=120;
            magicName="奔雷咒";
            finaldamage=0.1f*atkRole.roleMaxHp+0.8f*atkRole.roleMagicAttack;  // 0.1倍最大生命值+0.8倍法功伤害
        ...
        else
        {
            consumeMana=150;
            magicName="大力金刚拳";
            finaldamage=atkRole.roleDirectAttack+0.9f*atkRole.roleMagicAttack; // 1倍普攻伤害+0.9倍法伤

        }
        
        // 伤害波动，每次攻击造成的伤害范围为95%~105%
        float x1=UnityEngine.Random.Range(0.95f,1.05f);  
        finaldamage=finaldamage*x1;
    ...
    public Tuple<int,int,string,bool,bool> MagicWeapenAttackDamageCalculate(RoleInfo atkRole,RoleInfo dmgRole)
    {
            ...
            {
                magicName="九火神龙罩";
                finaldamage=atkRole.roleDirectAttack+dmgRole.roleHp*0.2f;  // 造成1倍物理伤害+0.2倍受伤者当前生命值伤害
                consumeMana=30;
            }
            else if(RandomDetermine(0.5f))
            {
            ...
            {
                magicName="生死金石";
                float atkrate=UnityEngine.Random.Range(0.03f,4f);
                finaldamage=atkrate*atkRole.roleDirectAttack; //造成0.03至4倍的输出，要么生，要么死！
                consumeMana=60;
    ...
    /// 更新战斗描述文本
    /// </summary>
    /// <param name="txt1">战斗描述</param>
    /// <param name="txt2">回合数</param>
    public void UpdateBattleDescText(string txt1,string txt2)
    {
        Text battleText=GameObject.Find("BattleDescription").GetComponentInChildren<Text>();
        Text roundText=GameObject.Find("RoundNum").GetComponentInChildren<Text>();

        battleText.text=txt1;
        roundText.text=txt2;
        Scrollbar battleTextScrollbar;
        battleTextScrollbar=GameObject.Find("BattleTextScrollbar").GetComponent<Scrollbar>();
        battleTextScrollbar.value=0f;  //确保每次输出的文本都显示在最下一行
